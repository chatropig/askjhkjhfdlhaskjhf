...
<head>
    ...
    <link rel="stylesheet" href="css/main.css?v=1770403388">
...
<script>
    ...
    <div class="dep-header-container">
        <div class="compact-card">
            <div class="cc-left">
                <span class="cc-label">Current Balance</span>
    ...
    <div class="amount-section" style="margin-top:15px;">
        ...
        <div class="input-wrapper">
            ...
            <input type="number" id="amount" class="clean-input" value="300" placeholder="0" onkeyup="updateButton()">
        </div>
        <div class="min-limit-text" id="minMsg">Minimum deposit is ₹300</div>

        <div class="chips-grid">
            <div class="d-chip active" onclick="setAmt(300, this)">300</div>
            <div class="d-chip" onclick="setAmt(500, this)">500</div>
            <div class="d-chip" onclick="setAmt(1000, this)">1000</div>
            <div class="d-chip" onclick="setAmt(2000, this)">2000</div>
            <div class="d-chip" onclick="setAmt(5000, this)">5000</div>
            <div class="d-chip" onclick="setAmt(10000, this)">10000</div>
        </div>
    </div>

    <div style="margin: 0 20px 10px; font-size:12px; font-weight:600; color:#8b949e;">PAY VIA</div>
    ...
    <div class="method-card">
        ...
        <i class="fas fa-check-circle m-check"></i>
    </div>

    <div class="trust-row">
        <div class="trust-item"><i class="fas fa-lock"></i> SSL Encrypted Payment</div>
    </div>

    <div class="pay-bar">
        <button class="btn-pay-secure" onclick="processDeposit()">
            Proceed to Pay ₹<span id="btnAmt">300</span>
        </button>
    </div>
    ...
    <script>
        ...
        function setAmt(val, el) {
            $('.d-chip').removeClass('active');
            $(el).addClass('active');
            $('#amount').val(val);
            updateButton();
        }
        ...
        function updateButton() {
            ...
            if(val < 300 && val !== '') {
                $('#minMsg').show();
                $('.btn-pay-secure').css('opacity', '0.5');
            } else {
                $('#minMsg').hide();
                $('.btn-pay-secure').css('opacity', '1');
            }
            $('#btnAmt').text(val ? val : '0');
        }
        ...
        function processDeposit() {
            ...
            if (!amount || amount < 300) {
                toastr.error("Minimum deposit is ₹300");
                $('#amount').focus();
                $('#minMsg').show();
                return;
            }
            ...
            btn.html('<i class="fas fa-spinner fa-spin"></i> SECURELY CONNECTING...').prop('disabled', true);
